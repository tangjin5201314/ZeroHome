...
- (void)adjustFrame
{
    ...
    // 挪动标签的位置
    CGPoint center = _footStatusLabel.center;
    center.y = _footImageView.center.y;
    _footStatusLabel.center = center;
}
...
- (void)setState:(RefreshState)state
{
	...
	switch (_state = state)
    {
        ...
        {
            _footStatusLabel.text = kReleaseToRefresh;
        ...
        {
			_footStatusLabel.text = kPullToRefresh;
            [_footImageView stopAnimating];
            [UIView animateWithDuration:0.2 animations:^{
        ...
        {
            _footStatusLabel.text = kRefreshing;
            NSMutableArray *images  = [[NSMutableArray alloc]init];
            for(int i=4;i<7;i++){
                [images addObject:[UIImage imageNamed:[NSString stringWithFormat:@"%d",i]]];
            }
            _footImageView.animationImages = images;
            _footImageView.animationDuration = .9f;
            _footImageView.animationRepeatCount = 0;
            [_footImageView startAnimating];
